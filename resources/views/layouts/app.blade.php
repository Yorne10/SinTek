...
@section('content')
    ...
    $isAppShell = in_array($routeName, [
        ...
        $prefix . '.secretary.notifications.send',
        $prefix . '.secretary.notifications.edit',
        $prefix . '.faq.categories',
        $prefix . '.faq.category.create',
        $prefix . '.faq.category.edit',
        $prefix . '.faq.questions',
        $prefix . '.faq.question.create',
        ...
        $prefix . '.secretary.worker-procedures',
        // Admin routes
    ...
    @elseif($isAuthShell)
        ...
        <script>
            document.addEventListener('DOMContentLoaded',
                    function() { // Ping server every 30 seconds to keep session alive on login/register pages                     setInterval(function () {                         fetch(window.location.href, {                             method: 'HEAD',                             headers: {                                 'X-CSRF-TOKEN': '{{ csrf_token() }}'                             }                         }).catch(e => console.error('Keep-alive failed', e));                     }, 30000);                 });
        </script>
