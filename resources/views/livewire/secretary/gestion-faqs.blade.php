...
<div>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <p class="mb-0">Administra las categorías y preguntas frecuentes que verán los trabajadores.</p>
        </div>
    </div>
        ...
        <div class="tab-pane fade show active" id="categories" role="tabpanel">
            <div class="card border-0 shadow mb-4">
                <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center">
                    <h2 class="fs-5 fw-bold mb-0">Categorías de FAQs</h2>
                        ...
                        <form wire:submit.prevent="saveCategory">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" wire:model="categoryName" class="form-control @error('categoryName') is-invalid @enderror" placeholder="Ej. Trámites generales">
                                    @error('categoryName') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-semibold">Orden</label>
                                    <input type="number" wire:model="categoryOrder" class="form-control @error('categoryOrder') is-invalid @enderror" min="0">
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea wire:model="categoryDescription" rows="2" class="form-control @error('categoryDescription') is-invalid @enderror" placeholder="Descripción breve de la categoría"></textarea>
                                    @error('categoryDescription') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-12">
                                    <button type="submit" class="btn btn-primary">
                                        {{ $editingCategoryId ? 'Actualizar' : 'Guardar' }} Categoría
                                    </button>
                                    <button type="button" class="btn btn-gray ms-2" wire:click="toggleCategoryForm">Cancelar</button>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <tbody>
                                @forelse ($categories as $category)
                                    <tr wire:key="category-{{ $category->faq_category_id }}">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-sm btn-info me-1" wire:click="editCategory({{ $category->faq_category_id }})" title="Editar">
                                                <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                                </svg>
                                            </button>
                                            <button type="button" class="btn btn-sm {{ $category->is_active ? 'btn-warning' : 'btn-success' }} me-1" wire:click="toggleCategoryStatus({{ $category->faq_category_id }})" title="{{ $category->is_active ? 'Desactivar' : 'Activar' }}">
                                                @if($category->is_active)
                                                    <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger delete-category-btn" data-category-id="{{ $category->faq_category_id }}" title="Eliminar">
                                                <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <form wire:submit.prevent="saveFaq">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select wire:model="selectedCategoryId" class="form-select @error('selectedCategoryId') is-invalid @enderror">
                                ...
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-semibold">Orden</label>
                                    <input type="number" wire:model="faqOrder" class="form-control @error('faqOrder') is-invalid @enderror" min="0">
                                    @error('faqOrder') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-12 mb-3">
                                    <label class="form-label fw-semibold">Pregunta</label>
                                    <input type="text" wire:model="faqQuestion" class="form-control @error('faqQuestion') is-invalid @enderror" placeholder="¿Cómo puedo...?">
                                    @error('faqQuestion') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                                <div class="col-12 mb-3">
                                    <label class="form-label fw-semibold">Respuesta</label>
                                    <textarea wire:model="faqAnswer" rows="4" class="form-control @error('faqAnswer') is-invalid @enderror" placeholder="Aquí va la respuesta detallada..."></textarea>
                                ...
                                <div class="col-12">
                                    ...
                                    <button type="button" class="btn btn-gray ms-2" wire:click="toggleFaqForm">Cancelar</button>
                                </div>
                            </div>
                        </form>
                    </div>
                            ...
                            <tbody>
                                @forelse ($faqs as $faq)
                                    <tr wire:key="faq-{{ $faq->faq_id }}">
                                        ...
                                        <td>
                                            <span class="fw-bold text-info">{{ $faq->category->name ?? 'Sin categoría' }}</span>
                                        </td>
                                        <td class="fw-semibold">{{ $faq->question }}</td>
                                        <td class="text-muted small text-truncate" style="max-width: 300px;">{{ $faq->answer }}</td>
                                        <td>
                                            @if($faq->is_active)
                                                <span class="fw-bold text-success">Activa</span>
                                            @else
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-sm btn-info me-1" wire:click="editFaq({{ $faq->faq_id }})" title="Editar">
                                                <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                                </svg>
                                            </button>
                                            <button type="button" class="btn btn-sm {{ $faq->is_active ? 'btn-warning' : 'btn-success' }} me-1" wire:click="toggleFaqStatus({{ $faq->faq_id }})" title="{{ $faq->is_active ? 'Desactivar' : 'Activar' }}">
                                                @if($faq->is_active)
                                                    <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                        <path fill-rule="evenodd" d="M13.477 14.89A6 6 0 015.11 6.524l8.367 8.368zm1.414-1.414L6.524 5.11a6 6 0 018.367 8.367zM18 10a8 8 0 11-16 0 8 8 0 0116 0z" clip-rule="evenodd"></path>
                                                    </svg>
                                            ...
                                            <button type="button" class="btn btn-sm btn-danger delete-faq-btn" data-faq-id="{{ $faq->faq_id }}" title="Eliminar">
                                                <svg class="icon icon-xs" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                                </svg>
                                            </button>
