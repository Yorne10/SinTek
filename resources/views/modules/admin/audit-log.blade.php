        ...
        <div class="d-block mb-4 mb-md-0">
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Bitácora de actividades</li>
                </ol>
            </nav>
    ...
    <div class="card card-body shadow border-0 table-wrapper table-responsive">
        <table class="table table-centered mb-0 rounded user-table align-items-center w-100" style="table-layout: fixed;">
            ...
            <tbody>
                @forelse($logs as $log)
                    <tr>
                        <td>
                            <span class="fw-bold text-gray-900 text-truncate d-inline-block w-100">{{ $log->user->name ?? 'Sistema' }}</span>
                        </td>
                        <td>
                            <span class="fw-normal text-dark text-truncate d-inline-block w-100">{{ $this->getRoleLabel($log->user->role ?? 'system') }}</span>
                        </td>
                        <td>
                            <span class="fw-normal text-truncate d-inline-block w-100">{{ \App\Services\ActivityLogger::getActionLabel($log->action ?? '') }}</span>
                        </td>
                        <td>
                            <span class="fw-normal text-truncate d-inline-block w-100" title="{{ $log->description ?? '-' }}">{{ $log->description ?? '-' }}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{ optional($log->date ?? $log->created_at)->format('d/m/Y H:i') }}</span>
                        </td>
                        <td>
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1">
                                    <button class="dropdown-item d-flex align-items-center view-log-detail" type="button"
        ...
        <div class="card-footer px-3 border-0 d-flex flex-column flex-lg-row align-items-center justify-content-between">
            ...
            <div class="fw-normal small mt-0 mt-lg-0 ms-lg-auto">
                Mostrando <b>{{ $logs->firstItem() ?? 0 }}</b> a <b>{{ $logs->lastItem() ?? 0 }}</b> de <b>{{ $logs->total() }}</b> registros
            </div>
        </div>
    </div>
    <script>
