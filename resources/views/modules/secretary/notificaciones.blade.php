    ...
    <div>
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
            <div class="d-block mb-4 mb-md-0">
                ...
                <p class="mb-0">Busca trabajadores por nombre o correo, agrégalos y envía un mensaje. También puedes
                    enviarlo a todos.</p>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <form wire:submit.prevent="sendNotification">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" role="switch" id="sendToAll"
                                    wire:model="sendToAll">
                                <label class="form-check-label" for="sendToAll">Enviar a todos los trabajadores</label>
                                <div class="text-muted small">Si activas esta opción no necesitas seleccionar usuarios.
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" placeholder="Nombre o correo"
                                    wire:model.live.debounce.400ms="workerSearch" wire:keydown.enter.prevent
                                    @if($sendToAll) disabled @endif>
                                <small class="text-muted d-block mt-1">Los resultados aparecen al escribir, no necesitas
                                    presionar Enter. Usa Agregar para añadirlos a la lista.</small>
                            </div>
                                ...
                                <div class="mb-3">
                                    <div class="list-group small mb-2">
                                        ...
                                        @else
                                            ...
                                            @empty
                                                <div class="list-group-item text-muted">Sin resultados para {{ $workerSearch }}.
                                                </div>
                                            @endforelse
                                        @endif
                                    </div>
                                        ...
                                        @forelse ($selectedUsers as $userId)
                                            ...
                                            <span class="badge bg-secondary d-flex align-items-center">
                                                ...
                                                <button type="button" class="btn-close btn-close-white btn-sm"
                                                    wire:click="removeUser({{ $userId }})" aria-label="Eliminar"></button>
                                            </span>
                                        @empty
                                            <span class="text-muted small">Aún no seleccionas usuarios.</span>
                                        @endforelse
                                    </div>
                                    @error('selectedUsers') <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                    @error('selectedUsers.*') <div class="invalid-feedback d-block">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input wire:model.defer="title" type="text"
                                    ...
                                    placeholder="Ej. Documento listo para recoger">
                            ...
                            <button type="button" id="sendNotificationBtn" class="btn btn-primary w-100 animate-up-2">
                                <span class="fas fa-paper-plane text-white me-2"></span>
                                Enviar notificación
                            </button>
                        </form>
                    </div>
                                    ...
                                    @forelse ($notifications as $notification)
                                        <tr wire:key="notification-{{ $notification->notification_id }}">
                                            ...
                                            <td>
                                                ...
                                                <div class="text-muted small">{{ $notification->user->email ?? 'N/A' }}
                                                </div>
                                            </td>
