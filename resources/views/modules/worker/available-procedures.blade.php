                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    @if ($process->category)
                                        <div class="mb-3">
                                            ...
                                            <span
                                                class="small fw-bold text-info text-capitalize">{{ $process->category }}</span>
                                        </div>
                                    @endif
                                    <div class="mt-auto d-grid">
                                        <button wire:click="startProcedure({{ $process->process_id }})"
                                            class="btn btn-gray-800" wire:loading.attr="disabled">
                                            <span wire:loading.remove
                                                wire:target="startProcedure({{ $process->process_id }})">
                                                @icon('add', 'me-2')
                                                Iniciar tramite
                                            </span>
                        ...
                        <div class="col-12">
                            <div class="card border-0 shadow">
                                <div class="card-body text-center py-5">
                                    ...
                                    <p class="text-gray-500 mb-0">En este momento no hay tramites que coincidan con tu
                                        busqueda.</p>
                                </div>
                            </div>
                        </div>
                ...
                @if ($processes->hasPages())
                    <div class="d-flex flex-column flex-lg-row align-items-center justify-content-between mt-4 gap-3">
                        ...
                        <div class="fw-normal small ms-lg-auto">
                            Mostrando <b>{{ $processes->firstItem() ?? 0 }}</b> a <b>{{ $processes->lastItem() ?? 0 }}</b>
                            de
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    if (window.Livewire) {
                        ...
                        Livewire.on('procedure-started', (event) => {
                            const detail = event || {};
                            Swal.fire({
                                icon: 'success',
                        ...
                        Livewire.on('procedure-error', (event) => {
                            const detail = event || {};
                            Swal.fire({
                                icon: 'error',
                                title: detail.title || 'Error',
                                text: detail.message || 'Ha ocurrido un error.',
                                confirmButtonText: 'Aceptar'
                            });
                        });
                    }
